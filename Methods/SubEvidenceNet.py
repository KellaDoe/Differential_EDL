...
class SubEvidenceNet(nn.Module):
    def __init__(self, model, num_class):
        super(SubEvidenceNet, self).__init__()
    ...
    def criterion(self, output, target, epoch_num, num_classes, annealing_step, device=None):
        ...
        sub_output = output - torch.min(output, dim=1, keepdim=True)[0]
        sub_evidence = torch.exp(sub_output)
        loss = edl_digamma_loss(sub_evidence, target, device=device)
